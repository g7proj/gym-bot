...
serve(async (req) => {
  ...
  try {
    ...
    const response = await myBookings(gymToken);
    ...
    const normalized = items
      ...
      .map((item) => ({
        ...
        date: String(item?.StartTime || '').slice(0, 10),
        startTime: String(item?.StartTime || '').slice(11, 16),
        endTime: String(item?.EndTime || '').slice(11, 16),
